
import {Container  } from "lucide-react";
const Navbar = () => {
  return (
    <div className="flex justify-center md:justify-between items-center p-4">
      <a href="#" className="flex items-center font-bold text-3xl md:text-xl"><Container  className="mr-2"/>MARC
      <span className="text-accent">DEV</span>
      </a>

      <ul className="hidden md:flex space-x-4">
        <li className="">
          <a href="#Accueil" className="btn btn-sm btn-ghost">Accueil</a>
        </li>

        <li className="">
          <a href="#Apropos" className="btn btn-sm btn-ghost">A propos</a>
        </li>

        <li className="">
          <a href="#Experience" className="btn btn-sm btn-ghost">Mes compétences</a>
        </li>

        <li className="">
          <a href="#Projects" className="btn btn-sm btn-ghost">Mes projets</a>
        </li>

        <li className="">
          <a href="#Contact" className="btn btn-sm btn-ghost">Contact</a>
        </li>
      </ul>
    </div>
  )
}

export default Navbar
